...
public class TokenHelper {
     ...
     */
    public synchronized static String checkTokenGZH(String appId, String appSecret, String agentid) {
        ...
        if (map == null) {
            ...
            AccessToken accessToken = WeChatHelper.getAccessToken(appId, appSecret, agentid);
            ...
            if (flag) {
                ...
                accessToken = WeChatHelper.getAccessToken(appId, appSecret, agentid);
                map.put("tokenKey", accessToken);
    ...
    public synchronized static String checkTicketAPI(String appId, String appSecret, String agentid) {
        ...
        if (map == null) {
            map = new HashMap<>();
            Long beginTime = new Date().getTime();
            JsApiTicket ticket = WeChatHelper.getJsapiTicket(appId, appSecret, agentid);
            ...
            if (flag) {
                Long beginNewTime = new Date().getTime();
                ticket = WeChatHelper.getJsapiTicket(appId, appSecret, agentid);
                map.put("ticketKey", ticket);
                map.put("beginTime", beginNewTime);
            }
            ticketId = ticket.getTicket();
